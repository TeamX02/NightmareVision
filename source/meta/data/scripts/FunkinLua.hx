...
import meta.data.scripts.FunkinScript;
...
class FunkinLua extends FunkinScript
{
	...
	public function new(script:String, ?name:String) {
		...
		Lua_helper.add_callback(lua, "noteTweenX", function(tag:String, note:Int, value:Dynamic, duration:Float, ease:String) {
			cancelTween(tag);
			if(note < 0) note = 0;
			var testicle:StrumNote = PlayState.instance.strumLineNotes[note % PlayState.instance.strumLineNotes.length];
		...
		Lua_helper.add_callback(lua, "exitSong", function(?skipTransition:Bool = false) {
			...
			if(PlayState.isStoryMode) Init.SwitchToPrimaryMenu(StoryMenuState);
